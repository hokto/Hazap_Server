...
def Search_route(APIKEY,start,goal):#最適ルートを取得する関数。
    url="https://map.yahooapis.jp/spatial/V1/shapeSearch?mode=circle&appid={key}&coordinates={start_lon},{start_lat} {goal_lon},{goal_lat}&sort=box&results=100&output=json"
    ...
    Making_route(APIKEY,str(start.lat)+","+str(start.lon),safty_places,str(goal.lat)+","+str(goal.lon))
...
def Making_route(APIKEY,start,list_via,goal):#最適ルート作成関数。(おそらく実際に通ったルートも同時に表示)
    url="https://map.yahooapis.jp/course/V1/routeMap?appid={apikey}&route={start_place},{via_places},{goal_place}"
    access_url=url.format(apikey=APIKEY,start_place=start,via_places=",".join(list_via),goal_place=goal)
